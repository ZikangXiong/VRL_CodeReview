...
def get_disjointness_constraint(x_dim, P1, P2, str_identifier):
    ...
    # str_identifier is just a new identifier for the new y variables
    # introduced
    A = P1[0]
    b = P1[1]
    C = P2[0]
    ...
    for i in range(x_dim):
        lst = [A[j][i] * y[j]
               for j in range(m1)] + [C[j][i] * y[j + m1] for j in range(m2)]
        list_of_constraints.append(Sum(lst) == 0)

    # Next assert f^T > 0
    lst = [b[j] * y[j]
           for j in range(m1)] + [d[j] * y[j + m1] for j in range(m2)]
    list_of_constraints.append(Sum(lst) > 0)
...
def get_next_constraint_point(x, A, B, K, x_dim, next_x):
    ...
    A_cl = A + B.dot(K)
    for i in range(x_dim):
        constraint = Sum([A_cl[i, j] * x[j]
                          for j in range(x_dim)]) == next_x[i]
        list_of_constraints.append(constraint)
    return And(list_of_constraints)
...
def get_avoid_constraint_rect(interval_list, avoid_list):
    # interval_list = [(lo_1, hi_1), (lo_2, hi_2), ..., (lo_k, hi_k)]
    ...
    for avoid in avoid_list:
        ...
        if avoid_flag:
            list_of_constraints.append(
                get_disjointness_constraint_rect(dim, interval_list, avoid_desc))
        else:
            list_of_constraints.append(
                get_disjointness_constraint_poly_incomplete(dim, corner_list, avoid_desc))
    return And(list_of_constraints)
...
def add_constraints(s, x, Theta, x_dim, A, B, K, target, avoid_list, avoid_list_dynamic, num_steps):
    ...
    # avoid_list is a list [(flag1, poly1), (flag2, poly2) ...  (flagk,
    # polyk)], where flagi = True means rectangle, o/w general polytope. if
    # flagi = True, thn polyi = list of x_dim pairs (hi, low). O/w it is (Ai,
    # bi).

    is_dynamic_list_None = (avoid_list_dynamic is None)

    s.push()
    s.add(get_avoid_constraint(
        x[0], avoid_list if is_dynamic_list_None else avoid_list + avoid_list_dynamic[0]))
    res = s.check()
    if (res == sat):
    ...
    for i in range(num_steps):
        ...
        safety_constraint = get_avoid_constraint(
            x[i + 1], avoid_list if is_dynamic_list_None else avoid_list + avoid_list_dynamic[i + 1])
...
def get_counterexample(s, x, num_steps):
    ...
    m = s.model()
    # print("model=", m)
    trajectory = [[(m[x[i][j]].numerator_as_long()) * 1.0 / (m[x[i][j]].denominator_as_long())
                   for j in range(x_dim)] for i in range(num_steps + 1)]
...
def bounded_z3(x0, initial_size, Theta, K, A, B, target, safe, avoid_list, avoid_list_dynamic, num_steps):
    ...
    # Constraits for resricting the verifier's attension only on a region near
    # the input
    initial_rectangle = (True, [(
        x0[i, 0] - initial_size[i], x0[i, 0] + initial_size[i]) for i in range(x_dim)])
    ...
    s_check = None
    if safe is None:
        s_check = add_constraints(
            s, x, Theta, x_dim, A, B, K, target, avoid_list, avoid_list_dynamic, num_steps)
    else:
        s_check = add_constraints_safety(
            s, x, Theta, x_dim, A, B, K, target, safe, num_steps)
...
def verify_controller_z3(x0, Theta, verification_oracle, learning_oracle, draw_oracle, continuous):
    if continuous:
        ...
        # Todo: more experiments to find a reasonable threshold.
        useInv = (x_dim <= 2)
        ...
        while((not Theta_has_been_covered) and number_of_steps_initial_size_has_been_halved < max_decrease_steps and num_iters < max_num_iters):
            ...
            while(draw):
                ...
                if (draw_oracle is not None):
                    ...
                    print(("If the the following trjectory socre is {} >= 0, then safe!".format(
                        test_result))  				)
                    if (test_result >= 0):
            ...
            if vresult:
                print(
                    "Verification algorithm finds the inducive invariant for the controller {}".format(vinv))
            ...
            if (number_of_steps_initial_size_has_been_halved == 0 and vresult):
                ...
                if (pvresult is None or pvinv is None or pinitial_size is None):
                    # K seems not good to the verifier; restart a new search
                    # for K.
                    number_of_steps_initial_size_has_been_halved = 0
                    K = learning_oracle(x0)
                    initial_size = np.array(
                    ...
                    if not (useInv):
                        ...
                        if type(initial_size) is np.ndarray:
                            for i in range(len(x0)):
                                x_i = x0[i, 0]
                                cover.append(
                                    (x_i - initial_size[i], x_i + initial_size[i]))
                        else:
                            for i in range(len(x0)):
                                x_i = x0[i, 0]
                                cover.append(
                                    (x_i - initial_size, x_i + initial_size))
