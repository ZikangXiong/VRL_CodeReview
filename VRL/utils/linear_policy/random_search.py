...
# Last Modified time: 2020-01-12 23:48:01
# -------------------------------
import numpy as np
import scipy.linalg
...
def random_search_linear_policy(env, x0, eq_err, N, T, x_min=None, x_max=None, continuous=False, timestep=.01, rewardf=None,
    ...
    return random_search_helper(f, d, p, env.Q, env.R, x0, eq_err, N, T, x_min, x_max, continuous, timestep, rewardf,
    ...
    for k in range(N):
        ...
        for j in range(batch_size):
            ...
            for sign in [-1, 1]:
                ...
                reward = 0
                for t in range(T):
                    u = np.dot(K + sign * explore_mag * V, np.vstack(
                        [x, [1]])) if bias else np.dot(K + sign * explore_mag * V, x)
                    # Use discrete or continuous semantics based on user's
                    # choice
                    if continuous:
                        x = x + timestep * (f(x, u)) + eq_err * np.random.randn(
                            d, 1) if coffset is None else x + timestep * (f(x, u) + coffset) + eq_err * np.random.randn(d, 1)
                    else:
                        x = f(x, u) + eq_err * np.random.randn(d, 1) if coffset is None else f(
                            x, u) + coffset + eq_err * np.random.randn(d, 1)
                    if rewardf is None:
