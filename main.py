...
def random_search_helper(f, d, p, Q, R, x0, eq_err, N, T, x_min=None, x_max=None, continuous=False, timestep=.01, rewardf=None,
    ...
    def policy_test(K):
        ...
        for t in range(T):
            ...
            if continuous:
                x = x + timestep * (f(x, u)) + eq_err * np.random.randn(d, 1) if coffset is None else x + \
                    timestep * (f(x, u) + coffset) + \
                    eq_err * np.random.randn(d, 1)
            else:
                x = f(x, u) + eq_err * np.random.randn(d, 1) if coffset is None else f(x,
                                                                                       u) + coffset + eq_err * np.random.randn(d, 1)
            if rewardf is None:
                reward += -np.dot(x.T, Q.dot(x)) - np.dot(u.T, R.dot(u))
            else:
    ...
    for k in range(N):
        ...
        for j in range(batch_size):
            ...
            for sign in [-1, 1]:
                ...
                reward = 0
                for t in range(T):
                    u = np.dot(K + sign * explore_mag * V,
                               np.vstack([x, [1]])) if bias else np.dot(K + sign * explore_mag * V, x)
                    # Use discrete or continuous semantics based on user's
                    # choice
                    if continuous:
                        x = x + timestep * (f(x, u)) + eq_err * np.random.randn(d, 1) if coffset is None else x + \
                            timestep * (f(x, u) + coffset) + \
...
def policy_gradient_helper(f, d, p, Q, R, x0, eq_err, N, T, x_min=None, x_max=None, continuous=False, timestep=.01, rewardf=None,
    ...
    for k in range(N):
        ...
        for j in range(batch_size):
            mini_batch += ((reward[j] - baseline) /
                           batch_size) * mb_store[:, :, j]
        ...
        effective_step_size = step_size * \
            np.sqrt(1 - beta2**(k + 1)) / (1 - beta1**(k + 1))
...
def learn_controller(A, B, Q, R, x0, eq_err, learning_method, number_of_rollouts, simulation_steps,
    ...
    if (learning_method == "lqr"):
        ...
        # K = lqr_gain(A,B,Q,R)
        print "K = {}".format(K)
        # print "double c[] = {%f, %f, %f, %f};" % (K[0,0], K[0,1], K[0,2],
        # K[0,3])
    elif (learning_method == "random_search"):
        K = random_search_linear_policy(A, B, Q, R, x0, eq_err, number_of_rollouts, simulation_steps, x_min, x_max, continuous,
                                        timestep, rewardf, explore_mag, step_size, coffset=coffset, bias=bias, unsafe_flag=unsafe_flag, lqr_start=lqr_start)
        print "K = {}".format(K)
    elif (learning_method == "random_search_2"):
        K = uniform_random_linear_policy(
            A, B, Q, R, x0, eq_err, number_of_rollouts, simulation_steps, x_min, x_max, continuous, timestep)
        print "K = {}".format(K)
    elif (learning_method == "policy_gradient"):
        K = policy_gradient_adam_linear_policy(A, B, Q, R, x0, eq_err, number_of_rollouts, simulation_steps,
                                               x_min, x_max, continuous, timestep, rewardf, explore_mag, step_size, coffset=coffset)
        print "K = {}".format(K)
    else:
...
def draw_controller_helper(f, K, x0, simulation_steps, names, continuous=False, timestep=.01, rewardf=None, coordination=None, coffset=None, bias=False):
    ...
    for t in time:
        ...
        if continuous:
            xk = xk + timestep * \
                (f(xk, uk)) if coffset is None else xk + \
                timestep * (f(xk, uk) + coffset)
        else:
...
def learn_shield(A, B, Q, R, x0, eq_err, learning_method, number_of_rollouts, simulation_steps, actor, x_min, x_max,
    ...
    if (learning_method == "random_search"):
        K = random_search_linear_policy(A, B, Q, R, x0, eq_err, number_of_rollouts, simulation_steps, x_min, x_max, continuous,
                                        timestep, shield_reward, explore_mag, step_size, coffset=coffset, bias=bias, unsafe_flag=unsafe_flag, lqr_start=lqr_start)
        print "K = {}".format(K)
    elif (learning_method == "random_search_2"):
        K = uniform_random_linear_policy(A, B, Q, R, x0, eq_err, number_of_rollouts,
                                         simulation_steps, x_min, x_max, continuous, timestep, shield_reward)
        print "K = {}".format(K)
    elif (learning_method == "policy_gradient"):
        K = policy_gradient_adam_linear_policy(A, B, Q, R, x0, eq_err, number_of_rollouts, simulation_steps,
                                               x_min, x_max, continuous, timestep, shield_reward, explore_mag, step_size, coffset=coffset)
        print "K = {}".format(K)
    else:
        print "Learning method {} is not found".format(learning_method)
    return np.matrix(K)
...
def learn_polysys_shield(f, ds, us, Q, R, x0, eq_err, learning_method, number_of_rollouts, simulation_steps, actor,
    ...
    K = random_search_helper(f, ds, us, Q, R, x0, eq_err, number_of_rollouts, simulation_steps, continuous=continuous, timestep=timestep,
                             rewardf=shield_reward, explore_mag=explore_mag, step_size=step_size, coffset=coffset, bias=bias, unsafe_flag=unsafe_flag)
...
class SelfDefinedNN:
    ...
    def __init__(self, para, structure, u_max=None):
        self.para = para
    ...
    def predict(self, obs):
        ...
        while i < len(self.structure):
            h = self.linear(self.para[i][0], self.para[i][1], h)
            s = self.structure[i]

            if i + 1 < len(self.structure) and self.structure[i + 1] == "normalization":
                i += 1
                self.normalization(h, *self.para[i])
    ...
    def normalization(self, x, beta, gamma, mean, variance, epsilon=1e-5):
        # 1e-5 is the default value in tflearn implementation
        # this value is used to avoid devide 0, and does not change in
        # training.
    ...
    def load(cls, model_path):
        para, activate = np.load(
            "{}/NN.npy".format(model_path), allow_pickle=True)
        scale = None
        if os.path.isfile("{}/u_max.npy".format(model_path)):
            scale = np.load("{}/u_max.npy".format(model_path))
