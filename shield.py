...
class Shield(object):
    ...
    def train_shield(self, learning_method, number_of_rollouts, simulation_steps, eq_err=1e-2, rewardf=None, testf=None, explore_mag=.04, step_size=.05, names=None,
        ...
        '''
                def verification_oracle_continuous(x, initial_size, Theta, K):
                    ...
                    if (discretization):
                        S0 = Polyhedron.from_bounds(
                            self.env.s_min, self.env.s_max)
                        self.O_inf = verify_controller_via_discretization(
                            Acl, self.env.timestep, self.env.x_min, self.env.x_max)
                        ...
                        for i in range(self.env.state_dim):
                            init.append("init" + str(i + 1) + " = (x[" + str(i + 1) + "] - " + str(
                                self.env.s_min[i, 0]) + ")*(" + str(self.env.s_max[i, 0]) + "-x[" + str(i + 1) + "])")
                        for i in range(self.env.state_dim):
                            initSOSPoly.append(
                                "@variable m Zinit" + str(i + 1) + " SOSPoly(Z)")
                        for i in range(self.env.state_dim):
                            init_cnstr.append(
                        ...
                        for i in range(self.env.state_dim):
                            ...
                            init.append("init" + str(self.env.state_dim + i + 1) + " = (x[" + str(
                                i + 1) + "] - (" + str(l) + "))*((" + str(h) + ")-x[" + str(i + 1) + "])")
                        for i in range(self.env.state_dim):
                            initSOSPoly.append(
                                "@variable m Zinit" + str(self.env.state_dim + i + 1) + " SOSPoly(Z)")
                        for i in range(self.env.state_dim):
                            init_cnstr.append(
                                " - Zinit" + str(self.env.state_dim + i + 1) + "*init" + str(self.env.state_dim + i + 1))
                        ...
                        if (self.env.unsafe):
                            ...
                            for i in range(self.env.state_dim):
                                mid = (self.env.x_min[
                                       i, 0] + self.env.x_max[i, 0]) / 2
                                radium = self.env.x_max[i, 0] - mid
                                unsafe.append(
                                    "unsafe" + str(i + 1) + " = (x[" + str(i + 1) + "] - " + str(mid) + ")^2 - " + str(pow(radium, 2)))
                                unsafeSOSPoly.append(
                                    "@variable m Zunsafe" + str(i + 1) + " SOSPoly(Z)")
                                unsafe_cnstr.append(
                                    " - Zunsafe" + str(i + 1) + "*unsafe" + str(i + 1))
                             # Now we have init, unsafe and sysdynamics for
                             # verification
                        sos = genSOSContinuousAsDiscreteMultipleUnsafes(
                            self.env.timestep, self.env.state_dim, ",".join(
                                dxdt(Acl)), "\n".join(init), "\n".join(unsafe),
                            "\n".join(initSOSPoly), "\n".join(unsafeSOSPoly), "".join(init_cnstr), unsafe_cnstr, degree=degree)
                        verified = verifySOS(
                            writeSOS("SOS.jl", sos), False, 900)
                        print verified
                ...
                result, resultList = verify_controller_z3(
                    x0, Theta, verification_oracle_continuous, learning_oracle_continuous, draw_oracle_continuous, continuous=True)
        ...
        # discrete
            ...
            if self.K_list == []:
                ...
                def verification_oracle_discrete(x, initial_size, Theta, K):
                    self.O_inf = verify_controller(np.asarray(self.env.A), np.asarray(self.env.B), np.asarray(
                        self.K), self.env.x_min, self.env.x_max, self.env.u_min, self.env.u_max)
                    min = np.array([[x[i, 0] - initial_size[i]]
                                    for i in range(self.env.state_dim)])
                    max = np.array([[x[i, 0] + initial_size[i]]
                                    for i in range(self.env.state_dim)])
                ...
                Theta = (self.env.s_min, self.env.s_max)
                result = verify_controller_z3(
                    x0, Theta, verification_oracle_discrete, learning_oracle_discrete, draw_oracle_discrete, continuous=False)
                print("Shield synthesis result: {}".format(result))
                if result:
                    self.save_shield(os.path.split(self.model_path)[0])
            else:
    ...
    """
        ...
        if self.K_list == []:
            ...
            def verification_oracle_continuous(x, initial_size, Theta, K):
                ...
                if (self.env.bound_x_min is not None and self.env.bound_x_max is not None):
                    for i in range(self.env.state_dim):
                        if (self.env.bound_x_min[i, 0] is not None and self.env.bound_x_max[i, 0] is not None):
                            bound.append("bound" + str(i + 1) + " = (x[" + str(i + 1) + "] - " + str(self.env.bound_x_min[
                                         i, 0]) + ")*(" + str(self.env.bound_x_max[i, 0]) + "-x[" + str(i + 1) + "])")
                    for i in range(self.env.state_dim):
                        if (self.env.bound_x_min[i, 0] is not None and self.env.bound_x_max[i, 0] is not None):
                            boundSOSPoly.append(
                    ...
                    for i in range(self.env.state_dim):
                        if (self.env.disturbance_x_min[i, 0] is not None and self.env.disturbance_x_max[i, 0] is not None):
                            disturbance.append("disturbance" + str(i + 1) + " = (d[" + str(i + 1) + "] - " + str(
                                self.env.disturbance_x_min[i, 0]) + ")*(" + str(self.env.disturbance_x_max[i, 0]) + "-d[" + str(i + 1) + "])")
                    for i in range(self.env.state_dim):
                        if (self.env.disturbance_x_min[i, 0] is not None and self.env.disturbance_x_max[i, 0] is not None):
                ...
                verified = verifySOS(
                    writeSOS("SOS.jl", sos), False, 900, aggressive=aggressive)
                print verified
    ...
    def load_shield(self, model_path, enable_jit):
        if self.env.continuous:
            ...
            self.initial_range_list = [initr for initr in loadK(
                model_path + "/initial_range.model.npy")]
            ...
            for K in self.K_list:
                O_inf = verify_controller(np.asarray(self.env.A), np.asarray(self.env.B), np.asarray(
                    K), self.env.x_min, self.env.x_max, self.env.u_min, self.env.u_max)
    ...
    def learn_shield_gd(self, lr=0.00001, epsoides=100, steps=1000):
        ...
        for ep in xrange(epsoides):
            ...
            for step in xrange(steps):
                u = self.actor.predict(np.reshape(
                    np.array(self.env.xk), (1, self.actor.s_dim)))
                grad += np.array(((K.dot(self.env.xk) -
                                   u).dot(self.env.xk.T)))[0]
...
def barrier_certificate_str2func(bc_str, vars_num, enable_jit=False):
    ...
    if enable_jit:
        ...
        exec(("@jit" + "(float64 ({}))" + "\ndef B({}): return {}")
             .format(("float64," * vars_num)[:-1], args_str, eval_str))
    else:
...
class SelfDefinedNN:
  def __init__(self, para, activate):
    self.para = para
  ...
  @classmethod
  def load(cls, model_name, nn_struct):
    para, activate = load_NN(model_name, nn_struct)
    return cls(para, activate)
